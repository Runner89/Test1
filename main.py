...
def webhook():
    ...
    try:
        ...
        sell_percentage = float(data.get("sell_percentage", 1.5))
        api_key = data.get("api_key")
        secret_key = data.get("secret_key")
        symbol = data.get("symbol", "BTC-USDT")
        position_side = data.get("position_side") or data.get("positionSide") or "LONG"
        firebase_secret = data.get("FIREBASE_SECRET")
        price_from_webhook = float(data.get("price"))
        beenden = str(data.get("beenden", "")).lower()

        base_asset = symbol.split("-")[0]

        # --- BALANCE ABFRAGEN ---
        balance_response = get_balance(api_key, secret_key, "USDT")
        ...
        # --- HEBEL SETZEN ---
        logs.append(f"Setze Hebel auf 8.0 für {symbol} ({position_side})...")
        leverage_result = set_leverage(api_key, secret_key, symbol, leverage=8.0, side=position_side)
        logs.append(f"Hebel gesetzt: {leverage_result}")

        # --- OFFENE LIMIT-ORDERS ABFRAGEN ---
        open_orders_response = get_open_orders(api_key, secret_key, symbol)
        logs.append(f"Open Orders: {open_orders_response}")

        open_sell_orders_exist = False
        if open_orders_response.get("code") == 0:
            for order in open_orders_response["data"].get("orders", []):
                if order.get("side") == "SELL":
                    open_sell_orders_exist = True
                    break

        # --- ORDERGRÖSSE BESTIMMEN ODER ABBRECHEN ---
        if open_sell_orders_exist:
            ...
            if beenden == "ja":
                ...
                logs.append("Beenden = ja und keine offene Limit-Order → Code wird abgebrochen.")
            ...
            # Keine offene Sell-Limit Order & kein beenden = "ja" → neue Ordergröße berechnen
        ...
        if usdt_amount <= 0:
            logs.append("Ordergröße <= 0, keine Order wird platziert.")
            return jsonify({
                "error": False,
                "symbol": symbol,
                "grund": "Ordergröße 0 oder kleiner, keine Order platziert.",
                "logs": logs
            })

        # --- MARKET-ORDER PLATZIEREN ---
        logs.append(f"Plaziere Market-Order mit {usdt_amount} USDT für {symbol} ({position_side})...")
        market_order_response = place_market_order(api_key, secret_key, symbol, usdt_amount, position_side)
        logs.append(f"Market-Order Antwort: {market_order_response}")

        if market_order_response.get("code") != 0:
            logs.append("Fehler bei Market-Order, Abbruch.")
            return jsonify({
                "error": True,
                "symbol": symbol,
                "grund": "Market-Order fehlgeschlagen",
                "logs": logs
            })

        # --- KAUFPREIS IN FIREBASE SPEICHERN ---
        logs.append(f"Kaufpreis speichern für {base_asset}: {price_from_webhook}")
        save_result = firebase_speichere_kaufpreis(base_asset, price_from_webhook, firebase_secret)
        logs.append(f"Kaufpreis gespeichert, Status: {save_result}")

        # --- LIMIT-ORDER (SELL) PLATZIEREN ---
        sell_price = price_from_webhook * (1 + sell_percentage / 100)
        logs.append(f"Berechne Sell-Limit-Preis: {price_from_webhook} * (1 + {sell_percentage}% ) = {sell_price}")

        limit_order_response = place_limit_order(api_key, secret_key, symbol, usdt_amount, "SELL", sell_price, position_side)
